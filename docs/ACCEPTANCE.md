...
|---|---|---|
| Engineering ready for merge | yes | S1–S7, E1–E4, B1–B2 and D1 verified at head `f691a29` with mutation evidence; three-platform CI green including the installed-binary smoke (run [35051853082](https://github.com/0merUfuk/beme/actions/runs/35051853082)); every review thread answered with evidence and resolved; an independent adversarial review's four majors fixed. Merging remains the owner's decision |
| Release verification complete | no | P1, P2, P4 and P5 are verified; P3 must be re-run on the final head (its defects are fixed but the zero-improvisation run has not been repeated) and P6 is an owner decision |
...
| P1 | Full test suite on macOS, Linux, Windows | CI run [35051853082](https://github.com/0merUfuk/beme/actions/runs/35051853082) green on code head `f691a29`: `test (macos-latest)`, `test (ubuntu-latest)`, `test-windows`, all three `installed-binary-smoke` jobs, and `private-data-scan`. Locally at the same head: build, vet (darwin/linux/windows), full suite, `-race` on app/durable/learning/storage/evalrunner, threat corpus 34/34, docs 15/15, validate 19/19, CI regression 7/7, scan clean | none | verified | the linked run; a later docs-only commit has its own run linked in PR #1 |
| P2 | Installed-binary smoke tests per OS at the release candidate | `installed-binary-smoke` green on macOS, Linux and Windows in run [35051853082](https://github.com/0merUfuk/beme/actions/runs/35051853082): `go install`, then doctor/build/status/preview/candidate/forget/purge --dry-run plus the threat-corpus and benchmark binaries, run from outside the repository in an isolated deployment | none | verified | the linked run |
| P3 | Fresh-checkout documentation test at the current head | run on `79797c0` by an independent agent: clone, isolated HOME, public docs only. Verdict was **no** — installing the CLI and registering a source were undocumented, so both had to be invented; every documented claim it could test held (pending-purge doctor finding, ledger/key restore-together failure on all surfaces, `candidate --config` isolation, make targets, macOS platform paths) | defects fixed at the current head: README install section and links, OPERATIONS "Registering a source" (descriptor + entry format), the two dead cross-references, the dead `source`/`profile-cmd` branch, and a usage block missing four working commands | implemented-unverified | re-run the test on the final head; verified means zero improvisations |
