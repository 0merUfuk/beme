...
| Release verification complete | no | P1, P2, P4 and P5 are verified. P3 ran twice independently: the core path is improvisation-free and 7 of 8 remaining items are confirmed at `7654d6c`; the eighth is fixed and directly verified, but certifying a full zero-improvisation run needs one more independent pass. P6 is an owner decision |
...
| P3 | Fresh-checkout documentation test at the current head | two independent passes. Pass 1 (`9a9a6fa`): the core install-to-deployment path was improvisation-free; eight findings remained, two of them documentation that was factually wrong about ingestion behavior. All eight were fixed. Pass 2 (`7654d6c`, a different fresh agent, clone + isolated `HOME`/`BEME_*`, public docs only) confirmed 7 of 8: `--help`/`--version` exit 0; the whole core path (doctor → register → build → status → preview → forget → purge dry-run) with zero improvisations; observation provenance; the real `type` mapping (a `constraint` entry becomes `unmapped_reference`, lands under guidance, `constraints: 0`); the real status rule (`draft` ingested, `deprecated` skipped); the mismatched-ledger surface matrix including both stated exceptions (`doctor` exit 0 reporting `policy_blocked`, `status` not ledger-gated); and workspace registration | the eighth — a harness-free MCP smoke command that lost a stdin race as written — is fixed and verified by direct execution (exit 0, `serverInfo` naming `beme`); the adapter table now names the two harnesses the subcommand accepts | implemented-unverified | one more independent pass to certify a full zero-improvisation run; everything it flagged is fixed |
